...
class User:
    ...
    def __init__(self, bias_stdev=0.2, eval_stdev=0.2, mode='pareto', frac=0.1, 
                 pareto_shape=1.4, gamma_shape=3, true_bias=None, true_precision=None, is_instructor=False):
        ...
        if not true_precision is None:
            self.prec = true_precision
        elif mode == 'bimodal':
    ...
    def is_instructors_neighbour(self):
        """For debugging purposes only"""
        for it in self.items:
            for neighbour in it.users:
                if neighbour != self and neighbour.is_instructor:
                    return True
        return False
